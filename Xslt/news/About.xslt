...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
 exclude-result-prefixes="msxsl">
	...
	<xsl:template match="NewsImages" mode="pos4Images">
		...
		<xsl:if test="(position() mod 7)&gt; 1 and (position() mod 7) &lt; 5">
			<div class="col-4">
				<a class="about-img big">
					<xsl:attribute name="href">
						<xsl:value-of select="ImageUrl" />
		...
		<xsl:if test="(position() mod 7) &gt; 5 and (position() mod 7) &lt; 8">
			<xsl:if test="(position() mod 7)=6 ">
				<xsl:text disable-output-escaping="yes">&lt;div class="row" &gt;</xsl:text>
			</xsl:if>
			<xsl:if test="(position() mod 7) = 6 or (position() mod 7) = 7">
